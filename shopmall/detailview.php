...
//receivers테이블에서 주문번호에 맞는 정보를 가져옴
$result = mysql_query("select * from receivers where ordernum='$ordernum'", $con);
$total = mysql_num_rows($result);

$session = mysql_result($result, 0, "session");
$sender = mysql_result($result, 0, "sender"); //보낸이
$receiver = mysql_result($result, 0, "receiver"); //받는이
$phone = mysql_result($result, 0, "phone"); //전화번호
$address = mysql_result($result, 0, "address"); //주소
$message = mysql_result($result, 0, "message"); //배송메세지
$buydate = mysql_result($result, 0,  "buydate"); //구매날짜
$status = mysql_result($result, 0, "status"); //주문 상태
